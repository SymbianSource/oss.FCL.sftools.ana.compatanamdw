...
TInt CTestSocket::UDP(  )
	...
	int error = recvfrom(sock_fd,line,KMaxLine,0,(sockaddr*)&sender_addr,&sender_len);
	if (error < 0)
		{
		ret = KReadError;
        goto close;
...
TInt CTestSocket::SendToTestFailCases()
	...
	int error = sendto(sock_fd, line, KMaxLine, atoi(paramets[5]), (sockaddr*)&dest_addr, sizeof(dest_addr) );
	if (error < 0)
		{
		ret = KErrNone;
		goto close;
...
TInt CTestSocket::SendMsgRecvMsg()
	...
	msg.msg_controllen = 0;
	msg.msg_flags = 0;
	int error = recvmsg(sock_fd,&msg,0);
	if (error < 0)
		{
		pthread_join(testThread, &threadRetValPtr);
		ret = KReadError;
...
TInt CTestSocket::TestGreaterThan16kReadWrite()
	...
	popen("c:\\sys\\bin\\greaterthan16kreadwriteserver.exe","r");
	sleep(5);
