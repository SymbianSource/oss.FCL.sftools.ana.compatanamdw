...
TBool CT_FsData::GetAttMask(const TDesC& aParameterName, const TDesC& aSection, TUint& aAttMask)
	{
	aAttMask=0;

	TPtrC	attMaskStr;
	TBool	ret=GET_MANDATORY_STRING_PARAMETER(aParameterName, aSection, attMaskStr);
	if ( ret )
		{
		if ( !ConvertToAttMask(attMaskStr, aAttMask) )
			{
			TInt	intTemp;
...
void CT_FsData::DoCmdDrive(const TDesC& aSection)
	{
	...
	for ( index=0; index<size; ++index )
		{
		if ( driveInfo.iDriveAtt&driveAttMask[index] )
			{
			INFO_PRINTF2(_L("DriveAtt %S ON"), &driveAttText[index]);
			}
		}
	INFO_PRINTF2(_L("TDriveInfo.iMediaAtt=0x%X"), driveInfo.iMediaAtt);
	TUint	mediaAttMask[]=
		{
	...
	for ( index=0; index<size; ++index )
		{
		if ( driveInfo.iMediaAtt&mediaAttMask[index] )
			{
	...
	if(errorCode != KErrNone)
		...
		if (GetMediaTypeFromConfig(aSection, mediaType))
			...
			if (driveInfo.iType != mediaType)
				{
				ERR_PRINTF3(_L("Drive has wrong type. actual: %d, expected: %d"), driveInfo.iType, mediaType);
				SetBlockResult(EFail);
				}
			}
		...
		if (GetBatteryStateFromConfig(aSection, batteryState))
			{
			//Checking that type of drive is equal to passed through config file.
			if (driveInfo.iBattery != batteryState)
				{
				ERR_PRINTF3(_L("BatteryState is wrong. actual: %d, expected: %d"), driveInfo.iBattery, batteryState);
				SetBlockResult(EFail);
				}
			}
		...
		if (GetMediaAttsFromConfig(aSection, mediaAtts))
			{
			//Checking that type of drive is equal to passed through config file.
			if ( (driveInfo.iMediaAtt&mediaAtts)==mediaAtts )
				{
		...
		if (GetDriveAttsFromConfig(aSection, driveAtts))
			{
			//Checking that type of drive is equal to passed through config file.
			if ( (driveInfo.iDriveAtt!=driveAtts)==driveAtts )
				{
...
void CT_FsData::DoCmdVolume(const TDesC& aSection)
	{
	...
	if (errorCode != KErrNone)
		...
		if (GET_OPTIONAL_STRING_PARAMETER(KVolumeLabel(), aSection, volumeLabel))
			...
			if (volumeLabel.Compare(iVolumeInfo.iName))
				{
				ERR_PRINTF1(_L("Wrong name of volume"));
				SetBlockResult(EFail);
				}
			}
		...
		TInt	tmp;
		TInt64	volumeSize;
		if (GET_OPTIONAL_INT_PARAMETER(KVolumeSize(), aSection, tmp))
			{
			volumeSize = tmp;
			//Checking that name of volume is equal to passed through config file.
			if (volumeSize == iVolumeInfo.iSize)
				{
				ERR_PRINTF1(_L("Wrong volume size"));
				SetBlockResult(EFail);
				}
			}
		...
		//Optional
		TInt64	freeSpace;
		if (GET_OPTIONAL_INT_PARAMETER(KVolumeFreeSpace(), aSection, tmp))
			{
			freeSpace = tmp;
			//Checking that name of volume is equal to passed through config file.
			if (freeSpace == iVolumeInfo.iFree)
				{
				ERR_PRINTF1(_L("Wrong free space amount"));
				SetBlockResult(EFail);
				}
			}
		...
		if (GET_OPTIONAL_INT_PARAMETER(KVolumeUniqueID(), aSection, tmp))
			{
			if(tmp<0)
				{
				ERR_PRINTF1(_L("ID in config should be not less than 0 "));
				SetBlockResult(EFail);
				}

			volumeUniqueID = (TUint)tmp;
			//Checking that name of volume is equal to passed through config file.
			if (volumeUniqueID == iVolumeInfo.iUniqueID)
				{
				ERR_PRINTF1(_L("Wrong unique ID"));
				SetBlockResult(EFail);
				}
			}
		}
