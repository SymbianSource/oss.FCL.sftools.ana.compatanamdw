...
namespace 
    ...
    // Addressbook service hosted inside SysOpen Digia intra
    _LIT8(KWSPContract,         "urn:nokia:test:addrbook:2004-09");
...
//    const TBool MultipleSimultanousRHttpSessionsSupported = EFalse;
    ...
    //_LIT8(KProxyHost,             "esprx00.nokia.com"); // mmattola(!): this proxy works only in Helsinki(!)
...
//  TEST TABLE
	    ...
	    ENTRY("SnapIdL -  Test for CSenServiceConnection SnapIdL ",CSenServiceConnectionBCTest::UT_CSenServiceConnectionSnapIdL),
		ENTRY("SetSnapIdL -  Test for CSenServiceConnection SetSnapIdL ",CSenServiceConnectionBCTest::UT_CSenServiceConnectionSetSnapIdL),
...
TInt CSenServiceConnectionBCTest::UT_CSenServiceConnectionSetSnapIdL( TTestResult& aResult )
{
	return UT_CSenServiceConnectionSnapIdL(aResult );
}
...
TInt CSenServiceConnectionBCTest::UT_CSenServiceConnection_SendL_Pattern_IAP2L( TTestResult& aResult )
    ...
    // TODO: Following send does not cause IAP to be prompted.
    //       HttpChannel should be changed to make IAP to be prompted
    //       after IAP has been once set.
    iServiceConnection->SendL(*pSOAPMessage);
    //StartActiveScheduler(1);
    //LOCAL_ASSERT(iCallback == ECallbackHandleMessage);
...
// TODO: improve http properties code setters to validate input (to leave if tag length == 0) AND then use KNullDesC8 as tag name in here to test it:
...
// TODO: improve http properties code setters to validate input (to leave if tag starts with number) AND then use KNullDesC8 as tag name in here to test it:
