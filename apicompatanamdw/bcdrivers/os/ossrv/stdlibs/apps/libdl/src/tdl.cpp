...
TVerdict CTestDlByName::TestSymLkupWithHandle()
{
	...
	/*open the dll*/
__UHEAP_MARK;	
	void* handle=dlopen("c:\\Sys\\Bin\\arithmeticoper1.dll", RTLD_LOCAL|RTLD_LAZY);
	TEST_COND( (handle!=NULL && dlerror()==NULL), _L("dlopen() with RTLD_LOCAL..."));

	/*open the same dll*/
	void* handle1=dlopen("c:\\Sys\\Bin\\arithmeticoper1.dll", RTLD_LOCAL|RTLD_LAZY);
...
//	TEST_COND((add==NULL && dlerror()!=NULL), _L("dlsym() using invalid ordinal number..."));
	...
	/*load an non-std dll*/
	void* handle2=dlopen("c:\\sys\\bin\\dll1.dll", RTLD_GLOBAL|RTLD_LAZY);
...
TVerdict CTestDlByName::TestSymLkupWithRTLD_DEFAULT()
{
	...
	/*open the dll*/
	void* handle=dlopen("c:\\Sys\\Bin\\arithmeticoper1.dll", RTLD_GLOBAL|RTLD_LAZY);
...
TVerdict CTestDlByName::TestLoadWithInvalidArg()
{
	...
	void* handle=dlopen("c:\\Sys\\Bin\\arithmeticoper5.dll", RTLD_GLOBAL|RTLD_LAZY);
	TEST_COND( (handle==NULL && dlerror()!=NULL), _L("dlopen() the non-existing dll..."));

	/*open the invalid dll*/
	void* handle1=dlopen("c:\\Sys\\Bin\\arithmeticoper5.dat", RTLD_GLOBAL|RTLD_LAZY);
...
TVerdict CTestDlByName::OpenNonStdDllOrdinalLookup()
	...
	mylib = dlopen("c:\\sys\\bin\\dll1.dll", RTLD_GLOBAL);
