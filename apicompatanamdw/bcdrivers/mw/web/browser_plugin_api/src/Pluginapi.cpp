...
// CONSTANTS
 ...
 _LIT(KPluginName,"S60 API Test Plugin");
 _LIT(KPluginDescription,"Example plug-in to test plug-in APIs");
...
static CPluginAPIInst* Inst( NPP aInstance )
    {
    CPluginAPIInst* inst = ( CPluginAPIInst* )aInstance->pdata;
    return inst;
    }
...
NPError PluginNew( 
    ...
    {
    CPluginAPIInst* inst = NULL;
    TRAPD( ret, inst = CPluginAPIInst :: NewL( aInstance ) );
    if ( ret )
        {
