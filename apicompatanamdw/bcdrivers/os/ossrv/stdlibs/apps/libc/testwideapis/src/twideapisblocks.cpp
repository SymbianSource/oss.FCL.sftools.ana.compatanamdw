...
// -----------------------------------------------------------------------------
//Function Name : wfreopen_valinvL
//API Tested : wfreopen
				   ...
				   if((errno == EEXIST) || (!ret_wmkdir))
						...
						wunlink(pathName) ;
	   			        ...
	   			        if((fp=wfreopen(pathName ,L"w+",stdout))==NULL) 
	 			        	...
	 			        	{
	 			        	if(ftell(fp) != printf("Output Redirected to file now"))
	 			        	  	{
	 			        	 	fclose(fp);	 			        	 
	 			        	 	wunlink(pathName);
	 			        	 	wrmdir(L"C:\\ggg");
...
/*
-------------------------------------------------------------------------------
Function Name		: CTestWideApi::getws_valL
API Tested			: getws
...
/*
-------------------------------------------------------------------------------
Function Name		: CTestWideApi::getws_nullL
API Tested			: getws
...
/*
-------------------------------------------------------------------------------
Function Name		: CTestWideApi::putws_val1L
API Tested			: putws
...
TInt CTestWideApi::wremove_val3() 
{
   ...
   if(!res)
	...
	wchar_t* dirName = (wchar_t *)malloc(2*sizeof(wchar_t));
	if(dirName==NULL)
		return KErrNoMemory;	
	dirName[0] = nameRead;	
	dirName[1] = L'\0';
    wchdir(L"C:\\TestFramework");   	
