...
void CT_FormatData::DoCmdNext(const TDesC& aSection)
/**
 * RFormat::Next
 */
	{
...
void CT_FormatData::RunL(CActive* aActive)
	{
	if ( aActive==iNext )
		{
		...
		if( err != KErrNone )
			...
			INFO_PRINTF2(_L("iCount %d"), iCountPckg());
...
void CT_FormatData::DoCancel(CActive* aActive)
	{
	if ( aActive==iNext )
		{
		TInt	err = aActive->iStatus.Int();
