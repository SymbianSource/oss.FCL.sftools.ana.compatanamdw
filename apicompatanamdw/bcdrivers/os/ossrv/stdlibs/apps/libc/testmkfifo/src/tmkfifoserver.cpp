...
CTestStep* CMkfifoTestServer::CreateTestStep(const TDesC& aStepName)
	...
	if(aStepName == KFifoCreateNULL)
		{
			testStep = new CTestMkfifo(aStepName);
		}
	if(aStepName == KFifoCreateInSystemDir)
		{
			testStep = new CTestMkfifo(aStepName);
		}
	if(aStepName == KFifoCreateNameTooLong)
		{
			testStep = new CTestMkfifo(aStepName);
		}
	if(aStepName == KFifoCreateNonExistantDir)
		{
			testStep = new CTestMkfifo(aStepName);
		}
	if(aStepName == KTMkfifoReaderWriter)
		{
			testStep = new CTestMkfifo(aStepName);
		}
	if(aStepName == KTMkfifoReaderWriter1)
		{
			testStep = new CTestMkfifo(aStepName);
		}
	    return testStep;
