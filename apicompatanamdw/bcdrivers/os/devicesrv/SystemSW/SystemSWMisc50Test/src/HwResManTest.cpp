...
void CHwResManTest::SendTestModuleVersion()
	...
	moduleName = _L("SystemSWMisc50Test.dll");
...
void CHwResManTest::VibraFeedbackModeChanged(CHWRMVibra::TVibraFeedbackModeState aMode)
    {
	RDebug::Print(_L("CHwResManTest - VibraFeedbackModeChanged to: %d"), aMode);
	...
	switch(aMode)
		...
		case CHWRMVibra::EVibraFeedbackModeUnknown:
		    {
			iLog->Log(_L("HwResManTest Vibra Feedback Mode changed to EVibraFeedbackModeUnknown"));
		...
		case CHWRMVibra::EVibraFeedbackModeON:
		    {
			iLog->Log(_L("HwResManTest Vibra Feedback Mode changed to EVibraFeedbackModeON"));
		...
		case CHWRMVibra::EVibraFeedbackModeOFF:
		    {
			iLog->Log(_L("HwResManTest Vibra Feedback Mode changed to EVibraFeedbackModeOFF"));
			
		    }
		    break;
							
		default:
			iLog->Log(_L("HwResManTest Vibra Feedback Mode changed to %d"), aMode);
    	}
    }
...
TInt CHwResManTest::RunReserveVibra( CHWRMVibra* iVibra, TBool aRestoreState, TBool aForceNoCCoeEnv)
{
	TInt errorStatus;	
	iLog->Log(_L("HwResManTest Vibra reserve request %d, %d"), aRestoreState, aForceNoCCoeEnv);
	TRAP(errorStatus, iVibra->ReserveVibraL(aRestoreState, aForceNoCCoeEnv));
	iLog->Log(_L("HwResManTest Vibra reservation, Error:%d"), errorStatus);
	return errorStatus;
}

void CHwResManTest::RunReleaseVibra( CHWRMVibra* iVibra)
{
	iVibra->ReleaseVibra();
	iLog->Log(_L("HwResManTest Vibra released"));
}
...
TInt CHwResManTest::PulseTestVibra( CHWRMVibra* iVibra, TInt howLong, TInt whatIntensity )
{
	TInt errorStatus;	
	TRAP(errorStatus, iVibra->PulseVibraL(howLong, whatIntensity));
	iLog->Log(_L("HwResManTest Vibra start for %d seconds with intensity %d, Error:%d"), howLong/1000, whatIntensity, errorStatus);
	return errorStatus;
}

TInt CHwResManTest::PulseTestVibra( CHWRMVibra* iVibra, TInt howLong )
{
	TInt errorStatus;	
	TRAP(errorStatus, iVibra->PulseVibraL(howLong));
	iLog->Log(_L("HwResManTest Vibra start for %d seconds with default intensity Error:%d"), howLong/1000, errorStatus);
	return errorStatus;
}

TInt CHwResManTest::PulseTestVibra( CHWRMVibra* iVibra )
{
	TInt errorStatus;	
	TRAP(errorStatus, iVibra->PulseVibraL());
	iLog->Log(_L("HwResManTest Vibra pulse with default duration and intensity Error:%d"), errorStatus);
	return errorStatus;
}
