...
// -----------------------------------------------------------------------------
// CConnMonBc::CConnMonBc
// C++ default constructor can NOT contain any code, that
// might leave.
// -----------------------------------------------------------------------------
//
CEConnPrefBc::CEConnPrefBc()
    {
    }

// -----------------------------------------------------------------------------
// CEConnPrefBc::ConstructL
...
void CEConnPrefBc::ConstructL()
    ...
    iStdLog = CStifLogger::NewL( KEConnPrefBcLogPath, KEConnPrefBcLogFile );
    iLog = iStdLog;

    _LIT( KLogStr, "econnprefbc.dll - Extended Connection Preferences API BC tests" );
    iLog->Log( KLogStr );
    }
...
// -----------------------------------------------------------------------------
//
CEConnPrefBc* CEConnPrefBc::NewL()
    {
    CEConnPrefBc* self = new (ELeave) CEConnPrefBc;
...
// -----------------------------------------------------------------------------
//
TInt CEConnPrefBc::InitL(
    TFileName& /*aIniFile*/,
    TBool /*aFirstTime*/ )
    {
    _LIT( KLogStr, "econnprefbc.dll - Initialized" );
    iLog->Log( KLogStr );
    return KErrNone;
    }

// -----------------------------------------------------------------------------
// CEConnPrefBc::GetTestCasesL
...
// -----------------------------------------------------------------------------
//
TInt CEConnPrefBc::GetTestCasesL(
    ...
    _LIT( KLogStr, "econnprefbc.dll - Loaded %d test cases" );
    iLog->Log( KLogStr, aTestCases.Count() );
    return KErrNone;
    }

// -----------------------------------------------------------------------------
// CEConnPrefBc::RunTestCaseL
...
// -----------------------------------------------------------------------------
//
TInt CEConnPrefBc::RunTestCaseL(
    ...
    if ( tmp.iMethod )
        ...
        if ( iAddTestCaseTitleToLogName )
            ...
            logFileName.Format( KEConnPrefBcLogFileWithTitle, &title );

            iTCLog = CStifLogger::NewL( KEConnPrefBcLogPath, logFileName );
            iLog = iTCLog;
            }
...
// -----------------------------------------------------------------------------
// CEConnPrefBc::OOMTestQueryL
// Used to check if a particular test case should be run in OOM conditions and
// which memory allocations should fail.
// -----------------------------------------------------------------------------
//
TBool CEConnPrefBc::OOMTestQueryL(
...
// -----------------------------------------------------------------------------
// CEConnPrefBc::OOMTestInitializeL
...
// -----------------------------------------------------------------------------
//
void CEConnPrefBc::OOMTestInitializeL(
        const TFileName& /*aTestCaseFile*/,
        const TInt /*aCaseNumber*/ )
    {
    }
...
// -----------------------------------------------------------------------------
// CEConnPrefBc::OOMHandleWarningL
...
// -----------------------------------------------------------------------------
//
void CEConnPrefBc::OOMHandleWarningL(
        const TFileName& /*aTestCaseFile*/,
        const TInt /*aCaseNumber*/,
        TInt& /*aFailNextValue*/ )
    {
...
// -----------------------------------------------------------------------------
//
void CEConnPrefBc::OOMTestFinalizeL(
        const TFileName& /*aTestCaseFile*/,
        const TInt /*aCaseNumber*/ )
    {
    }
...
//-----------------------------------------------------------------------------
// CEConnPrefBc::SendTestModuleVersion
// Method used to send version of test module
//-----------------------------------------------------------------------------
//
void CEConnPrefBc::SendTestModuleVersion()
    {
    ...
    _LIT( KTestDllName, "econnprefbc.dll" );
...
//-----------------------------------------------------------------------------
// CEConnPrefBc::SetTestCaseResult
// Method used to set test case result according to error count.
//-----------------------------------------------------------------------------
//
TInt CEConnPrefBc::SetTestCaseResult( TTestResult& aResult, const TInt aErrorCount )
    {
