...
void CT_FsData::DoCmdGetDir(const TDesC& aSection)
	...
	if (GET_OPTIONAL_STRING_PARAMETER(KEntryAttMask(), aSection, entryAttMask))
		{
		if ( !GetAttMask(KEntryAttMask(), aSection, attMask))
            {
           	dataOk = EFalse;
           	}
		...
		{
		dataOk = EFalse;
		ERR_PRINTF3(_L("%S or %S must be declared !!!"), &KEntryAttMask, &KTUidType);
		}
...
void CT_FsData::DoCmdAtt(const TDesC& aSection)
	...
	if (GET_MANDATORY_STRING_PARAMETER(KName(), aSection, name))
		...
		if (err != KErrNone)
			...
			TPtrC	strTestAttMask;
			TUint	testAttMask = 0;
			if (GET_OPTIONAL_STRING_PARAMETER(KEntrySetAttMask(), aSection, strTestAttMask))
				{
				if ( !GetAttMask(KEntrySetAttMask(), aSection, testAttMask))
		            {
		            ERR_PRINTF2(_L("Unknown attribute %S"), &strTestAttMask);
		            SetBlockResult(EFail);
		            }
		        else
		        	{
					...
					if (GET_OPTIONAL_BOOL_PARAMETER(KIsSetAtt(), aSection, testIsSet))
						{
						INFO_PRINTF2(_L("TEST attributes %S must be set"), &strTestAttMask);
						}
					else
						{
						INFO_PRINTF2(_L("TEST attributes %S must not be set"), &strTestAttMask);
						}
					...
					if ( (attValue&testAttMask)==testAttMask )
						...
						if ( !testIsSet )
							{
							ERR_PRINTF2(_L("All bits not set %S"), &strTestAttMask);
							SetBlockResult(EFail);
							}
						}
						...
						if ( testIsSet )
							{
							ERR_PRINTF2(_L("Some bits set %S"), &strTestAttMask);
							SetBlockResult(EFail);
							}
						}
					else
...
void CT_FsData::DoCmdSetAtt(const TDesC&  aSection)
	...
	TUint	setAttMask = 0;
	GetAttMask(KEntrySetAttMask(), aSection, setAttMask);

	//	Reads attributes to be clear from ini file.
	TUint	clearAttMask = 0;
	GetAttMask(KEntryClearAttMask(), aSection, clearAttMask);
	...
	if (GET_MANDATORY_STRING_PARAMETER(KName(), aSection, name))
		...
		if (err != KErrNone)
			...
			for	(TInt i = 0; i < size; i++)
				...
				if (GET_OPTIONAL_BOOL_PARAMETER(iniParam[i], aSection, iniCompare))
					...
					if (!arrFunctions[i] != !iniCompare)
						{
						ERR_PRINTF4(_L("Error compare attribute %S, %d, %d"), &iniParam[i], arrFunctions[i], iniCompare);
						SetBlockResult(EFail);
						}
					}
				}
			...
			if (GET_OPTIONAL_STRING_PARAMETER(KTime(), aSection, iniTimeStr))
				...
				//	Assigns a date and time contained in a descriptor.
				TTime	iniTime;
				User::LeaveIfError(iniTime.Set(iniTimeStr));
...
void CT_FsData::DoCmdSetEntry(const TDesC& aSection)
	...
	//	Reads attributes to be set from ini file.
	TUint	setAttMask = 0;
	if ( !GetAttMask(KEntrySetAttMask(), aSection, setAttMask) )
		{
		dataOk = EFalse;
		}
	...
	//	Reads attributes to be clear from ini file.
	TUint	clearAttMask = 0;
	if ( !GetAttMask(KEntryClearAttMask(), aSection, clearAttMask) )
		{
		dataOk = EFalse;
		}
...
TBool CT_FsData::ConvertToAttMask(const TDesC& aAttMaskStr, TUint& aAttMask)
	{
	TBool ret = ETrue;
