...
default resolution behaviour of only loading Content Access Agents (CAAs)
from the ROM drive. Compile time value is 0 - default resolution behaviour. 
When non-zero CAF will load all CAA visible to the ECOM service regardless of 
the drive. In the emulator platform the default resolution behaviour is the 
same as when this variable is non-zero, therefore loading all CAAs.
