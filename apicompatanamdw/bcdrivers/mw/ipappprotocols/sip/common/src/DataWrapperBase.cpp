...
TBool CDataWrapperBase::GetTUint32FromConfig(const TDesC& aSectName, const TDesC& aKeyName, TUint32& aResult)
	{
	TPtrC	result;
	TBool	ret=EFalse;
	TRAPD(err, ret=GetCommandStringParameterL(aSectName, aKeyName, result));
	...
	if ( ret )
		{
		TLex	lex(result);
		ret=(lex.Val(aResult,EDecimal)==KErrNone);
		}
...
TBool CDataWrapperBase::GetTUintFromConfig(const TDesC& aSectName, const TDesC& aKeyName, TUint& aResult)
	{
	TPtrC	result;
	TBool	ret=EFalse;
	TRAPD(err, ret=GetCommandStringParameterL(aSectName, aKeyName, result));
